    ...
    public HashMap<MobEffect, Float> getEffects(int level, ItemStack stack) {
        /*if (generatedEffects == null && worldSeed != 0) {
            ...
            if (!availableEffects.isEmpty()) {
                ...
                MysticAlchemy.LOGGER.info("PotionIngredientRecipe.getEffects: Generating " + count + " effects.");
                for (int i = 0; i < count && !availableEffects.isEmpty(); i++) {
                    ...
                    MysticAlchemy.LOGGER.info("PotionIngredientRecipe.getEffects: Selected effect " +
                            ForgeRegistries.MOB_EFFECTS.getKey(effect) + " with strength " + strength);
                }
            } else {
                MysticAlchemy.LOGGER.warn("PotionIngredientRecipe.getEffects: No available effects in registry!");
            }
        }
		*/
